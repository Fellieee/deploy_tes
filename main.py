...
with mx_customer:
    ...
    st.metric("Number of Customer", value=curr_cust, delta=f'{cust_diff_pct:.2f}%')

with mx_profit_pct:
    curr_pp = data.loc[data['order_year']==CURR_YEAR, 'profit_pct'].values[0]
    prev_pp = data.loc[data['order_year']==PREV_YEAR, 'profit_pct'].values[0]

    pp_diff_pct = 100.0 * (curr_pp - prev_pp) / prev_pp

    st.metric("Profit Percentage", value=f'{format_big_number(curr_pp)}%', delta=f'{pp_diff_pct:.2f}%')
...
st.header("Profit vs Sales Performance")
